...
class FHFU(nn.Module):
    ...
    def __init__(self,inchax1,inchax2,num_classes=21843):
    ...
    def forward(self, images, masks=None):  
        ...
        x_ppl, x_xpl=torch.split(images, (self.inchax1, self.inchax2), dim=1)
        # 将输入图像通过模型的主要架构（即Unet）进行前向传播，得到logits。  
        # logits = self.arc(images)  
        images = self.DBSEG(x_ppl, x_xpl)  # (B, n_patch, hidden)
